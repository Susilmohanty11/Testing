...
void run_one_iteration_prover(int iteration_number, char buffer[], int buffer_size,
		const char outputFile[], unsigned char *userInput){
...
void run_one_iteration_verifier(int iterationNummer, char buffer[], int buffer_size,
		const char inputFile[],unsigned char *userInput){
	//printf("Running verifier...\n");
	char string_pass[80],string_system[80];
	int j=0;
	//printf("Iterations of SHA: %d\n", NUM_ROUNDS);
	...
	for(int i=0;i<8;i++) { //FIXME should be 5 for SHA1?
		//printf("%08x\n", y[i]);
		sprintf(string_pass,"%08x",y[i]);
		//printf("%c,%c,%c,%c\n",string_pass[0],string_pass[0],userInput[j],userInput[j+1]);
		strcat(string_system,string_pass);
		
	}
	for(int i=0;i<strlen(userInput);i++)
	{
	//		printf("%c",userInput[i]);
			if(userInput[i]!=string_system[i])
			{	
				printf("wrong proof\n");
...
int main_proof(char *command,unsigned char *userInput,char *filename) {
/*	if (argc<=1){
	...
	for(int iter=0; iter<repetitions; iter++){
		...
		if (run_prover){
			 //55 is max length as we only support 447 bits = 55.875 bytes
			/*if (standard_in != 0){
			...
			run_one_iteration_prover(iter, file_in_memory, buffer_size,filename,userInput);
		}
		if (run_verifier){
			//run verifier
			run_one_iteration_verifier(iter, file_in_memory, buffer_size,filename,userInput);
		}
